...
You can run the application with `make run junta [ARGS...]`.
...
- `docker.image` is the basename: `junta`
- `docker.image.base` is the base/testing image name: `junta_base`
- `docker.image.deploy` is the workflow-specific deployment image name: `junta_<workflow>`
